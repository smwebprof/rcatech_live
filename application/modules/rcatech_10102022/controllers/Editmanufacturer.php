...
class Editmanufacturer extends MX_Controller {
	...
	public function index()
	{
		...
		if (@$_POST) {
        	...
        	$resultdata = $this->General_master->UpdateManufacturerdata($_POST);

        	########################### Log Activity ######################################
            $this->load->model('Activity_master');
            $params['module'] = 'editmanufacturer';
            $params['date_time'] = $dt;
        	...
        	if ($resultdata) {
 				$module = BASE_PATH."Viewmanufacturer/";
                redirect($module);
        	}
        	...
        	$result = $this->General_master->getManufacturerById($id);
        	//print_r($result);exit;

			$data['title'] = 'RCAinet Tech - Editmanufacturer';
			$data['manufacturer_data'] = $result;
			$data['layout_body']='editmanufacturer';
