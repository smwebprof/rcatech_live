...
class Edititemmaster extends MX_Controller {
	...
	public function index()
	{
		...
		if (@$_POST) {
        	...
        	$resultdata = $this->Item_master->updateItem($this->input->post());

        	########################### Log Activity ######################################
            $this->load->model('Activity_master');
            $params['module'] = 'additemmaster';
            $params['date_time'] = $dt;
        	...
        	if ($resultdata) {  
 				$redirecturl = BASE_PATH."Viewitemmaster?msg=1";
                redirect($redirecturl); 
            }

		} else { 

			$result = $this->Item_master->getItemdataById($id);
			//print_r($result);exit;

			$departmentdata = $this->Department_master->getDepartmentdata();
			//print_r($departmentdata);exit;

			$data['title'] = 'RCAinet Tech - Item Master';
			$data['item_data'] = $result;
			$data['departmentdata'] = $departmentdata;
			$data['layout_body']='edititemmaster';
